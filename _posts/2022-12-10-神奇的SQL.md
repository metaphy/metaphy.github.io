...
下面以例子解释SQL的编写，主要参考日本作者Mick的《SQL进阶教程（第2版）》，一个容易被人看不上的非常土的书名，但其实内容极好，豆瓣9.1评分。所有例子和SQL均在MySQL8上执行通过。
...
select city,
  sum(case when gender = 'M' then population else 0 end) as Popu_M,
  sum(case when gender = 'F' then population else 0 end) as Popu_F
from DEC_POPU
group by city;

-- update use case...when...else...end
update DEC_ORDER set PRICE = case
  when PRICE > 5000 then PRICE - 2000
  when PRICE < 1000 then PRICE + 300
  else PRICE
  end;

-- rows -> columns
select course_id, course_name,
  case when course_id in (select course_id from DEC_COURSE_SCHEDULE where schedule_month = '2022-05') then 'o' else 'x' end as 'May',
  case when course_id in (select course_id from DEC_COURSE_SCHEDULE where schedule_month = '2022-06') then 'o' else 'x' end as 'June',
  case when course_id in (select course_id from DEC_COURSE_SCHEDULE where schedule_month = '2022-07') then 'o' else 'x' end as 'July'
from DEC_COURSE;

-- another method
select c.course_name,
  case when exists (select course_id from DEC_COURSE_SCHEDULE cs where c.course_id = cs.course_id and cs.schedule_month = '2022-05') then 'o' else 'x' end as May,
  case when exists (select course_id from DEC_COURSE_SCHEDULE cs where c.course_id = cs.course_id and cs.schedule_month = '2022-06') then 'o' else 'x' end as June,
  case when exists (select course_id from DEC_COURSE_SCHEDULE cs where c.course_id = cs.course_id and cs.schedule_month = '2022-07') then 'o' else 'x' end as July
from DEC_COURSE c;

--if there is only one Gender record, select it, else select the 'M' Gender one
select city, case when count(*) = 1 then max(population) else max(case when gender = 'M' then population else -1 end) end as PopuMain
from DEC_POPU
group by city;

-- columns are City Total, Province Total, Country Total
select gender
  , sum(population) as Total
...
-- find the max of x,y,z. it equals to 'select GREATEST(x,y,z) from...
select name, case when max1 > z then max1 else z end as max2
from (
  SELECT name, case when x > y then x else y end as max1, z
  from DEC_XYZ
) t1;

-- another way to the same result as above
SELECT name, 
  case when x > (case when y > z then y else z end) then x else (case when y > z then y else z end) end as max1
from DEC_XYZ;

--display by the name with special order
SELECT * FROM DEC_XYZ
order by case name
  when 'b' then 1
  when 'a' then 2
...
--Window
select server, sample_date as cur_date,
  MIN(sample_date) OVER (partition by server order by sample_date ASC rows between 1 preceding and 1 preceding) as latest_date
from DEC_SERVER_LOAD;

--Window, lastest date and latest loads value
select server, sample_date as cur_date, loads as cur_loads,
  min(sample_date) OVER W as last_date,
  min(loads) OVER W as last_loads
from DEC_SERVER_LOAD
Window W as (partition by server order by sample_date ASC rows between 1 preceding and current row);

--Undefined Window is the whole table
select server, sample_date, SUM(loads) over() as sum_loads
from DEC_SERVER_LOAD;

select server, sample_date, SUM(loads) over(partition by server) as sum_loads
from DEC_SERVER_LOAD;

--self join, remove (a, a) (b, b) ...
select A.name as name1, B.name as name2
from DEC_XYZ A cross join DEC_XYZ B
  on A.name <> B.name
order by A.name;

-- self join, keep only 1 for the (a, b) & (b, a)
select A.name as name1, B.name as name2
from DEC_XYZ A inner join DEC_XYZ B
  on A.name < B.name
order by A.name;

-- search for the duplicated records. Keep the min ID as primary
select * from DEC_PRODUCT p1
  where id >
    (select min(p2.id) from DEC_PRODUCT p2 where p1.product_name = p2.product_name and p1.price = p2.price);

-- search for the duplciated records. Keep the Max(id) 
select id from DEC_PRODUCT
except  
select max(id) from DEC_PRODUCT
group by product_name, price;

-- search all the duplicated by product name & price
select * from DEC_PRODUCT p1
where exists (
  select 1 from (
  ...
  where p2.cnt > 1 and p1.product_name = p2.product_name and p1.price = p2.price
)
...
in (
  select id from (  -- make sure you search the id again
    select id from DEC_PRODUCT p1
    where id > (select min(p2.id) from DEC_PRODUCT p2 where p1.product_name = p2.product_name and p1.price = p2.price)
  ) t1
);
...
INSERT INTO DEC_XYZ(name, x, y, z) VALUES('a', 1, 2, 3);
INSERT INTO DEC_XYZ(name, x, y, z) VALUES('b', 5, 5, 2);
INSERT INTO DEC_XYZ(name, x, y, z) VALUES('c', 4, 7, 1);
INSERT INTO DEC_XYZ(name, x, y, z) VALUES('d', 3, 3, 8);
