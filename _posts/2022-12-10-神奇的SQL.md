...
SELECT p1.fruit_name, p1.price,
  (SELECT COUNT(p2.price) FROM dec_fruits p2 WHERE p2.price > p1.price) + 1 AS rank_1,
  (SELECT COUNT(DISTINCT p2.price) FROM dec_fruits p2 WHERE p2.price > p1.price) + 1 AS rank_2_dense
FROM dec_fruits p1
ORDER BY rank_1;
```
...
SELECT sale_year, 
  ...
  SUM(sale) OVER (ORDER BY sale_year rows between 1 preceding AND 1 preceding) AS last_year_sale,
  sale - SUM(sale) OVER (ORDER BY sale_year rows between 1 preceding AND 1 preceding) as diff
FROM dec_sale;
```
