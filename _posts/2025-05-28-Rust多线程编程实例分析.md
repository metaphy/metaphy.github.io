...
因此4.2处 self.0 就是拿到的recevier锁。  

4.3处，guard.recv().ok()方法是把Result转为Option，因为Iterator的next要求返回Option

4.4处clone，因为线程要求所有权，索引需要把receiver做clone，又因为它是Arc，所以本质就是把引用计数加了1. 另外需要注意是加mut，因为next方法需要&mut self。 

所以，总体功能，一个sender线程发送20这消息到channel，而3个receiver线程处理这20条消息。因为系统每次有不同调度，所以每次结果都不尽相同。下面是某次运行结果：
